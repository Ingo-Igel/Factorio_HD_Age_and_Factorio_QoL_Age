...
local function asteroid_variation(asteroid_type, suffix, size)
	return {
		color_texture = {
			filename = base .. asteroid_type .. "/" .. asteroid_sizes[size] .. "/" .. "asteroid-" .. asteroid_type .. "-" .. asteroid_sizes[size] .. "-colour-" .. suffix .. ".png",
			size = sizes_resolution[size],
			scale = graphics_scale[size] * 0.5
		},
		shadow_shift = { 0.25 * size, 0.25 * size },
		normal_map = {
			filename = base .. asteroid_type .. "/" .. asteroid_sizes[size] .. "/" .. "asteroid-" .. asteroid_type .. "-" .. asteroid_sizes[size] .. "-normal-" .. suffix .. ".png",
			premul_alpha = false,
			size = sizes_resolution[size],
			scale = graphics_scale[size] * 0.5
		},
		roughness_map = {
			filename = base .. asteroid_type .. "/" .. asteroid_sizes[size] .. "/" .. "asteroid-" .. asteroid_type .. "-" .. asteroid_sizes[size] .. "-roughness-" .. suffix .. ".png",
			premul_alpha = false,
			size = sizes_resolution[size],
			scale = graphics_scale[size] * 0.5
		}
